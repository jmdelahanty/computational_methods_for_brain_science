...
% -------------------------------
% Added by Shreesh Mysore, Mar 19, 2022:
% All data points belonging to class 1 can be identified using these probabilities as:
% class1ind = find(P(:,1)>P(:,2));  
% Similarly, all data points belonging to class 2 can be identified as:
% class2ind = find(P(:,2)>=P(:,1)); 
% 
% (Alternatively, one can just use the linear scores that form the basis of the probabilities: 
% class1ind = find(L(:,1)>L(:,2));  class2ind = find(L(:,2)>=L(:,1)); )
% -----------------------------
%
%
